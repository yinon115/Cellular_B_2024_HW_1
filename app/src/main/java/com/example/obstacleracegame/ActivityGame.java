...
public class ActivityGame extends AppCompatActivity {
    ...
    private ShapeableImageView[][] game_IMG_kb;
    ...
    private void checkIfCollision() {
        if(game_IMG_kb[3][playerLocation].getVisibility()==View.VISIBLE){
            if(life >1){
                life--;
            }
            else if (life==1){
    ...
    private void moveObstaclesMatrixDown() {
        for(int j=0;j<3;j++) {
            for (int i = 3; i > 0; i--) {
                game_IMG_kb[i][j].setVisibility(game_IMG_kb[i - 1][j].getVisibility());
            }
        }
        for(int i=0;i<3;i++){
            game_IMG_kb[0][i].setVisibility(View.INVISIBLE);
        }
        game_IMG_kb[0][r.nextInt(3)].setVisibility(View.VISIBLE);
    ...
    private void initObstaclesMatrix(){
        game_IMG_kb = new ShapeableImageView[][]{
        ...
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 3; j++) {
                game_IMG_kb[i][j].setVisibility(View.INVISIBLE);
            }
        }
    }
